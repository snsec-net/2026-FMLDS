"""Evaluate a trained HMT variant on T25-26 test."""
import argparse
import pickle
import sys
import time
...
from HMT.HMT_model import HybridModifiedTransformer
from HMT.HMT_dataloader import HMTDataset
...
def main() -> None:
    ...
    ckpt = path_artifacts.joinpath(f"HMT_T24_{args.variant}_30days.pt")
    ...
    val_loader = DataLoader(HMTDataset(val_df, bigram_dict), batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)

    val_out, val_lab = [], []
    with torch.no_grad():
        for cx, bx, y in tqdm(val_loader, desc=f"[Val] {args.variant}", leave=False):
            cx, bx = cx.to(device, non_blocking=True), bx.to(device, non_blocking=True)
            out = torch.sigmoid(model(cx, bx).squeeze(1)).cpu()
            val_out.append(out)
            val_lab.append(y)
    val_out = torch.cat(val_out)
    val_lab = torch.cat(val_lab)
    fpr_v, tpr_v, thr_v = roc_curve(val_lab.numpy(), val_out.numpy())
    theta = float(thr_v[(tpr_v - fpr_v).argmax()])
    print(f"[HMT/{args.variant}] theta = {theta:.4f}", flush=True)
    ...
    print(f"[HMT/{args.variant}] DGA test: {len(dga_test_df):,}, benign test: {len(benign_test_df):,}", flush=True)

    print(f"[HMT/{args.variant}] Computing LLR on test benigns...", flush=True)
    ...
    test_loader = DataLoader(HMTDataset(test_df, bigram_dict), batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)

    t0 = time.time()
    all_preds, all_labels = [], []
    with torch.no_grad():
        for cx, bx, y in tqdm(test_loader, desc=f"[Test] {args.variant}", leave=False):
            cx, bx = cx.to(device, non_blocking=True), bx.to(device, non_blocking=True)
            out = torch.sigmoid(model(cx, bx).squeeze(1)).cpu()
            all_preds.append(out)
            all_labels.append(y)
    print(f"[HMT/{args.variant}] Inference: {time.time()-t0:.1f}s", flush=True)
    ...
    def metrics(y_true, y_pred, name):
        ...
        print(f"  [{name}] n={len(y_true):>8,} acc={acc:.4f} prec={prec:.4f} rec={rec:.4f} f1={f1:.4f} FPR={fpr:.4f} FNR={fnr:.4f}", flush=True)
    ...
    print(f"\n[HMT/{args.variant}] FULL TEST:", flush=True)
    full_row = metrics(all_labels, pred_labels, name="full")

    print(f"\n[HMT/{args.variant}] LLR-CLEANED TEST:", flush=True)
    ...
    out = pd.DataFrame([dict(model="HMT", variant=args.variant, theta=theta, **full_row), dict(model="HMT", variant=args.variant, theta=theta, **cln_row)])
    out_path = path_results.joinpath(f"results_HMT_{args.variant}_byYear.csv")
    out.to_csv(out_path, index=False)
    print(f"[HMT/{args.variant}] Saved metrics to {out_path}", flush=True)
