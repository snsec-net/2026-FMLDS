"""Evaluate a trained MIT variant on T25-26 test, with full and LLR-cleaned views."""
import argparse
import sys
import time
...
def score_llr(domains, M_D, M_B):
    return domains.map(lambda d: llr_mod.score(d, M_D, M_B))
...
def main() -> None:
    ...
    model = MIT().to(device)
    ckpt = path_artifacts.joinpath(f"MIT_T24_{args.variant}_30days.pt")
    ...
    with torch.no_grad():
        for X, y in tqdm(val_loader, desc=f"[Val] {args.variant}", leave=False):
    ...
    print(f"[{args.variant}] theta = {theta:.4f}", flush=True)
    ...
    print(f"[{args.variant}] DGA test: {len(dga_test_df):,}, benign test: {len(benign_test_df):,}", flush=True)

    print(f"[{args.variant}] Computing LLR on test benigns...", flush=True)
    ...
    benign_test_df["llr"] = score_llr(benign_test_df["domain"], M_D, M_B)
    print(f"[{args.variant}] Test benign LLR > tau: {(benign_test_df['llr'] > tau).mean():.4%}", flush=True)
    ...
    print(f"[{args.variant}] Inference: {time.time()-t0:.1f}s", flush=True)
    ...
    print(f"\n[{args.variant}] FULL TEST:", flush=True)
    full_row = metrics(all_labels, pred_labels, name="full")

    print(f"\n[{args.variant}] LLR-CLEANED TEST:", flush=True)
    ...
    out = pd.DataFrame([dict(variant=args.variant, theta=theta, **full_row), dict(variant=args.variant, theta=theta, **cln_row)])
    out_path = path_results.joinpath(f"results_MIT_{args.variant}_byYear.csv")
    out.to_csv(out_path, index=False)
    print(f"[{args.variant}] Saved metrics to {out_path}", flush=True)
