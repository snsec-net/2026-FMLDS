"""NYU_DGA detector training with three benign-filtering variants."""
import argparse
import sys
import time
...
def main() -> None:
    ...
    print(f"[NYU/{args.variant}] LLR threshold tau = {tau:+.4f}")
    ...
    print(f"[NYU/{args.variant}] Benign kept: {len(kept):,} / {n_total:,} ({len(kept)/n_total:.2%})")

    kept = kept[target_cols]
    benign_train_df, benign_val_df = train_test_split(kept, test_size=0.1, random_state=42)
    print(f"[NYU/{args.variant}] Benign train: {len(benign_train_df):,}, Val: {len(benign_val_df):,}")
    print(f"[NYU/{args.variant}] DGA    train: {len(dga_train_df):,}, Val: {len(dga_val_df):,}")
    ...
    print(f"[NYU/{args.variant}] Device: {device}")
    ...
    best_epoch = 0
    model_path = path_artifacts.joinpath(f"NYU_T24_{args.variant}_30days.pt")
    ...
    for epoch in range(args.epochs):
        ...
        dt = time.time() - t0
        print(f"[NYU/{args.variant}] ep {epoch+1:2d}/{args.epochs} [{dt:.1f}s] train={avg_train_loss:.4f} val={avg_val_loss:.4f} acc={accuracy:.4f} prec={precision:.4f} rec={recall:.4f} f1={f1:.4f} theta={theta:.4f}", flush=True)

    print(f"\n[NYU/{args.variant}] Best epoch {best_epoch} (val_loss={best_val_loss:.4f}); saved to {model_path}")
